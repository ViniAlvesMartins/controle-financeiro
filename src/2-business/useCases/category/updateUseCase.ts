...
export const UpdateUseCaseToken = 'UpdateUseCase'

@Service(UpdateUseCaseToken)
export class UpdateUseCase extends BaseUseCase<ICategory> {
  ...
  async exec (categoryId: number, name: string): Promise<CategoryEntity> {
    this._logger.info(`class: ${UpdateUseCase.name} | method: exec | message: starting useCase execution`)
    ...
    this._logger.info(`class: ${UpdateUseCase.name} | method: exec | message: finishing useCase execution`)
