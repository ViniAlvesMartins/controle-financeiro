...
export const CreateUseCaseToken = 'CreateUseCase'

@Service(CreateUseCaseToken)
export class CreateUseCase extends BaseUseCase<ICategory> {
  ...
  async exec (name: string): Promise<CategoryEntity> {
    this._logger.info(`class: ${CreateUseCase.name} | method: exec | message: starting useCase execution`)
    ...
    this._logger.info(`class: ${CreateUseCase.name} | method: exec | message: finishing useCase execution`)
